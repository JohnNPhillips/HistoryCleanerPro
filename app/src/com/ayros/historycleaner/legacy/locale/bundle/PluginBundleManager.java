...
package com.ayros.historycleaner.legacy.locale.bundle;
...
import com.ayros.historycleaner.legacy.locale.Constants;
...
public final class PluginBundleManager
{
	 ...
	 */
	public static final String BUNDLE_EXTRA_STRING_CLEAN_PROFILE = "com.ayros.historycleaner.legacy.extra.CLEAN_PROFILE"; //$NON-NLS-1$
	 ...
	 */
	public static final String BUNDLE_EXTRA_INT_VERSION_CODE = "com.ayros.historycleaner.legacy.extra.INT_VERSION_CODE"; //$NON-NLS-1$
	...
	public static boolean isBundleValid(final Bundle bundle)
	{
		 ...
		 */
		if (!bundle.containsKey(BUNDLE_EXTRA_STRING_CLEAN_PROFILE))
		{
			if (Constants.IS_LOGGABLE)
			{
				Log.e(Constants.LOG_TAG,
					String.format("bundle must contain extra %s", BUNDLE_EXTRA_STRING_CLEAN_PROFILE)); //$NON-NLS-1$
			}
			return false;
		}
		...
		if (bundle.keySet().size() != 2)
		{
			...
			{
				Log.e(Constants.LOG_TAG,
					String.format("bundle must contain 2 keys, but currently contains %d keys: %s",
		...
		if (TextUtils.isEmpty(bundle.getString(BUNDLE_EXTRA_STRING_CLEAN_PROFILE)))
		{
			if (Constants.IS_LOGGABLE)
			{
				Log.e(Constants.LOG_TAG,
					String.format("bundle extra %s appears to be null or empty.  It must be a non-empty string",
						BUNDLE_EXTRA_STRING_CLEAN_PROFILE)); //$NON-NLS-1$
			}
			return false;
		}
		...
		if (bundle.getInt(BUNDLE_EXTRA_INT_VERSION_CODE, 0) != bundle.getInt(BUNDLE_EXTRA_INT_VERSION_CODE, 1))
		{
			...
			{
				Log.e(Constants.LOG_TAG,
					String.format("bundle extra %s appears to be the wrong type.  It must be an int",
	...
	public static Bundle generateBundle(final Context context, final String message)
	{
		...
		result.putString(BUNDLE_EXTRA_STRING_CLEAN_PROFILE, message);
